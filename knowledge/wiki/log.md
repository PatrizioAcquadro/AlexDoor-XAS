# Project Milestones

A concise record of durable changes. Routine edits, lint and superseded attempts
belong in Git history, not in agent context.

| Milestone | Outcome | Reference |
|---|---|---|
| B0 study closeout | 550 matched episodes, sixteen seed-0 training cells, 576 successful rollouts; no winner and one unresolved force-review event. | `7f1fc8c`; experiment pages |
| B1 design | Purdue seven-joint visuoproprioceptive benchmark, held-out identities and expert-normalized progress. | `128c569` |
| 4.0 runtime | Purdue full-pose control, distal-contact classification and synchronized head RGB-D commissioned on GPU. | `59d59e1` |
| 4.1 common setup | Four synthetic doors, paired sustained opening, frozen base/tool/contact/probe. | `bdedefd` |
| Dataset strategy | Replicator changes observations; Mimic is conditional on a demonstrated motion-coverage gap. | `d2c584d` |
| 5.0 infrastructure | Shared recipes, conversion and isolated preparation gates before URL intake. | `d12f0b2` |
| 5.0 intake | 32 prepared doors: 29 redistributable, two local-only, one private/noncommercial; expert qualification pending. | `9c16e3d` |
| 2026-09-24 asset cleanup | Three canonical records and independent source/final payload folders; acquired results preserved without real-door revalidation. | `7a1ffb1` |
| 2026-09-24 B0 retirement | Removed obsolete compatibility/orchestration and local datasets/scenes; preserved reusable numerical components and B1 runtime. | `c83cf5e` |
| 2026-09-24 documentation cleanup | One current status, compact corpus/workflow page and historical conclusions; future specifications and raw research preserved. | Current documentation milestone |
